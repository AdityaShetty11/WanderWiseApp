...
import { CATEGORY_CONFIG, COVER_COLORS, formatDate, formatDateShort, daysBetween } from "../utils/helpers";
import { Activity, ActivityCategory, AISuggestion, Trip, TripDay } from "../types";
import { generateId } from "../utils/helpers";

// ── Mock trip for UI preview ─────────────────────────────
function buildMockTrip(): Trip {
  const days: TripDay[] = [
    {
      id: "d1", date: "2025-06-01", order: 0,
      activities: [
        { id: "a1", title: "Senso-ji Temple", time: "09:00", category: "sightseeing", location: "Asakusa, Tokyo", notes: "Arrive early to avoid crowds.", createdAt: "" },
        { id: "a2", title: "Ramen at Ichiran", time: "12:30", category: "food", location: "Shibuya", createdAt: "" },
        { id: "a3", title: "Shibuya Crossing", time: "15:00", category: "sightseeing", location: "Shibuya, Tokyo", createdAt: "" },
        { id: "a4", title: "Yakitori dinner at Torikizoku", time: "19:30", category: "food", location: "Shinjuku", notes: "Book in advance on weekends.", createdAt: "" },
      ],
    },
    {
      id: "d2", date: "2025-06-02", order: 1,
      activities: [
        { id: "a5", title: "Tsukiji Outer Market", time: "08:00", category: "food", location: "Tsukiji, Tokyo", createdAt: "" },
        { id: "a6", title: "teamLab Borderless", time: "11:00", category: "sightseeing", location: "Azabudai Hills", notes: "Book tickets online — sells out fast.", createdAt: "" },
      ],
    },
    { id: "d3", date: "2025-06-03", order: 2, activities: [] },
    { id: "d4", date: "2025-06-04", order: 3, activities: [] },
  ];
  return {
    id: "1", title: "Summer in Japan", destination: "Tokyo · Kyoto · Osaka",
    startDate: "2025-06-01", endDate: "2025-06-14",
    coverColor: "#378ADD", description: "A week of temples, ramen and bullet trains.",
    userId: "u1", createdAt: "", days,
  };
}
...
export default function TripDetailPage() {
  ...
  const [trip, setTrip] = useState<Trip>(buildMockTrip());
  ...
  const totalActs = trip.days.reduce((a, d) => a + d.activities.length, 0);
  const daysWithActs = trip.days.filter(d => d.activities.length > 0).length;
  ...
  function deleteActivity(dayId: string, actId: string) {
    setTrip(prev => ({
      ...
      days: prev.days.map(d =>
        d.id !== dayId ? d : { ...d, activities: d.activities.filter(a => a.id !== actId) }
      ),
    }));
  ...
  function addActivityFromSuggestion(sug: AISuggestion, dayId: string) {
    const act: Activity = {
      id: generateId(), title: sug.name, time: sug.bestTime,
      category: sug.category, location: sug.location, createdAt: new Date().toISOString(),
    };
    setTrip(prev => ({
      ...prev,
      days: prev.days.map(d =>
        d.id !== dayId ? d : { ...d, activities: [...d.activities, act] }
      ),
    }));
    setAddedSugs(prev => new Set(prev).add(sug.name));
  }

  const filteredSugs = MOCK_SUGGESTIONS.filter(s =>
    aiFilter === "All"         ? true :
    aiFilter === "Hidden gems" ? s.isHiddenGem :
    s.category === aiFilter.toLowerCase()
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <AppNav backLabel="My trips" backTo="/dashboard" />
            ...
            <div className="min-w-0">
              ...
              <div className="flex flex-wrap gap-2 items-center">
                {[
                  ...
                  <span key={l} className="text-[11px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">{l}</span>
                ))}
              </div>
              {trip.description && (
      ...
      {addDayId && (
        <AddActivityModal
          day={trip.days.find(d => d.id === addDayId)!}
          ...
          onSave={act => {
            setTrip(prev => ({
              ...prev,
              days: prev.days.map(d =>
                d.id !== addDayId ? d : { ...d, activities: [...d.activities, act] }
              ),
            }));
            setAddDayId(null);
          }}
          ...
          <div className="bg-white rounded-t-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
            ...
            <div className="px-5 pb-3 border-b border-gray-100">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 mt-2.5">
                {(["All", "Sightseeing", "Food", "Transport", "Hidden gems"] as AIFilter[]).map(f => (
                  <button
                    key={f}
                    onClick={() => setAIFilter(f)}
                    className={`text-xs px-3 py-1.5 rounded-full whitespace-nowrap border transition-colors flex-shrink-0 ${
                      aiFilter === f ? "bg-gray-900 text-white border-gray-900" : "bg-transparent border-gray-200 text-gray-500 hover:bg-gray-50"
                    }`}
                  >
                    {f}
                  </button>
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden relative">
        ...
        <div className="px-5 py-3.5 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
          <div className="text-xs">
            {title
              ? <span className="text-green-600 flex items-center gap-1"><svg width="12" height="12" viewBox="0 0 12 12" fill="none"><circle cx="6" cy="6" r="5.5" stroke="currentColor" strokeWidth="1"/><path d="M3.5 6l2 2 3-3" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/></svg>Ready to save</span>
